...
Rook::Rook(int c, int r, GameColor color) {
...
void Rook::setImage() {
	if (this->_pieceColor == GameColor::White) {
		this->_pieceTexture.loadFromFile("images/pieces/WhiteRook.png");
	}
	else {
		this->_pieceTexture.loadFromFile("images/pieces/BlackRook.png");
	}
...
Square Rook::getCurrentPlace() {
	return this->_currentPlace;
}

GameColor Rook::getPieceColor() {
	return this->_pieceColor;
}

void Rook::setSelected() {
	this->_isSelected = true;
}

void Rook::unsetSelected() {
	this->_isSelected = false;
}

void Rook::getPML(PieceBoard pb[][8]) {
	//four cases:
	//x + 1, y
	//x - 1, y
	//x, y + 1
...
void Rook::moveToNewPlace(int c, int r) {
	this->_currentPlace.change(c, r);
}